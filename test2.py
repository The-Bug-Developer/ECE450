################################################################
#                                                              #
# Zachary DeLuca                                               #
# ECE 450                                                      #
# Exam 2                                                       #
# Due: Oct 06                                                  #
#                                                              #
################################################################
import numpy as np                                             #
import matplotlib . pyplot as plt                              #
import scipy as sp                                             #
import scipy . signal as sig                                   #
import pandas as pd                                            #       
import time                                                    #
import math                                                    #
import cmath                                                   #
from scipy . fftpack import fft , fftshift                     #
################################################################

from math import pi, exp, cos, sin, log, sqrt
from control import margin
from control import tf

gain = 1
lead = 1
lag = 1
poles = 1

Range = 1000
fi = 86
phim = (pi/180)*fi
alpha = (1+sin(phim))/(1-sin(phim))
dbshift = -10*np.log10(alpha)

n1 = [0,0 ,10]
d1 = [1, 3, -10]

if gain:
    K = 45
    num = np.convolve(K,n1)
    den = d1
    print('Gain:',K)
else:
    num = n1
    den = d1


w = np.linspace(0.1,1000,num=10000)
system = sig.lti(n1,d1)
w, Hmag, Hphase = sig.bode(system,w)

wm = 1

for i in range(len(Hmag)):
    if(dbshift>0):
        if((Hmag[i]>dbshift*0.95) and (Hmag[i]<dbshift*1.15)): 
            wm = w[i]
            #print(Hmag[i],dbshift,'True')
            break
        else: 
            #print(Hmag[i],dbshift,'False')
            wm =wm 
    if(dbshift<0):
        if((Hmag[i]<dbshift*0.95) and (Hmag[i]>dbshift*1.15)): 
            wm = w[i]
            #print(Hmag[i],dbshift,'True')
            break
        else: 
            #print(Hmag[i],dbshift,'False')
            wm =wm 

p = sqrt(alpha)*wm
z = wm/sqrt(alpha)

# First Gc
n2 = [1,z]
d2 = [1,p]
    
if lead:
    K = p/z
    num = K*np.convolve(num,n2)
    den = np.convolve(den,d2)
    print('Lead: p:',p,'z:',z,'wm:',wm)
else:
    num = num
    den = den

#Lag addition
if lag:
    p = 10
    z = p/1000
    n2 = [1,p]
    d2 = [1,z]
    num = np.convolve(num,n2)
    den = np.convolve(den,d2)
    print('Lag: p:',p)
else:
    num = num
    den = den


if poles:
    n = 0.02
    n2 = [1,n]
    d2 = [1,0]
    num = np.convolve(num,n2)
    den = np.convolve(den,d2)
    print('Poles: n =',n)
else:
    num = num
    den = den



w = np.linspace(0.1,Range,num=1000)
system = sig.lti(num,den)
w, Hmag, Hphase = sig.bode(system,w)
gm, pm, wg, wp = margin(Hmag,Hphase,w)
# wp  freq for phase margin at gain crossover (gain = 1)
# pm phase maring

plt.subplot(211)
plt.semilogx(w,Hmag,'k')
plt.semilogx(w,Hmag,'k')
plt.axis([ .1, Range, -80, 80])
#plt.xticks([1,10,30,100,1000])
plt.ylabel('|H| dB',size = 12)
plt.text(.3,-40,'$\omega$p = {}'.format(round(wp,1)),fontsize=12)
plt.title('Bode Comp')
plt.grid(which='both')

for n in range(Range):
    if Hphase[n] > 0:
        Hphase[n] = Hphase[n] - 360

plt.subplot(212)
plt.semilogx(w,Hphase,'k')
plt.axis([ .1, Range, -180,0])
plt.yticks([-180,-90,0])


plt.xlabel('$\omega$ (rad/s)')
plt.ylabel('Phase (degrees)',size=12)
plt.text(.3,-150,'pm = {}'.format(round(pm,0)),fontsize=12)
plt.grid(which='both')
plt.savefig('Test2.1.png',dpi=300)
plt.show()

""" Time portion """

dt = 0.01
NN = 500
TT = np.arange(0,NN*dt,dt)
step = np.zeros(NN)
ramp = np.zeros(NN)
parabola = np.zeros(NN)
errS = np.zeros(NN)
errR = np.zeros(NN)
errP = np.zeros(NN)

for i in range(NN):
    step[i] = 1.0
    ramp[i] = (dt*i)
    parabola[i] = (dt*i)**(2)
    
denCL = np.add(num,den)

t1, y1, x1 = sig.lsim((num,denCL),step,TT)
t2, y2, x2 = sig.lsim((num,denCL),ramp,TT)
t3, y3, x3 = sig.lsim((num,denCL),parabola,TT)

for i in range(NN):
    errS[i] = step[i] - y1[i]
    errR[i] = ramp[i] - y2[i]
    errP[i] = parabola[i]  - y3[i]    


plt.figure(figsize=(11,8.5))
plt.suptitle("Step and Velocity Response and Errors",size=25)
plt.subplot(4,2,1)
plt.title('Time Domain Function',size = 15)
plt.plot(TT,y1,'k--',label='y1(t)')
plt.plot(TT,step,'k',label='u(t)')
plt.axis([0,1,0.9,1.1])
plt.ylabel('step')
plt.xlabel('t (sec)')
#plt.yticks([0,.9,1.1,1.5])
plt.legend()
plt.grid()

plt.subplot(4,2,2)
plt.title('Time Domain Error',size = 15)
plt.plot(TT,errS,'k',label='error')
plt.legend()
plt.axis([0,1,-0.03,0.03])
#plt.yticks([0,0.02,.05,.1])
plt.grid()

plt.subplot(4,2,3)

plt.plot(TT,y2,'k--',label='y2(t)')
plt.plot(TT,ramp,'k',label='r(t)')
plt.xlabel('t (sec)')
plt.ylabel('ramp')
plt.legend()
plt.axis([0,1,-3,3])
plt.grid()

plt.subplot(4,2,4)
plt.plot(TT,errR,'k',label='error')
plt.legend(loc=4)
plt.xlabel('t (sec)')
plt.axis([0,1,-0.05,.05])

plt.grid()
plt.savefig('Test2.2.png',dpi=300)
"""
plt.subplot(6,2,5)
plt.plot(TT,y3,'k--',label='y3(t)')
plt.plot(TT,parabola,'k',label='parab(t)')
plt.xlabel('t (sec)')
plt.ylabel('parabola')
plt.legend()
plt.grid()

plt.subplot(6,2,6)
plt.plot(TT,errP,'k',label='error')
plt.xlabel('t (sec)')
plt.legend()
plt.grid()
plt.savefig('time_plots.png',dpi=300)

plt.show()
"""